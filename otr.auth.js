 ...
 * 2012-11-12 now include old MAC keys
 */
...
Otr.Auth = (function () {
		...
		*/
		...
		consumeDataMessage: function (msg) {
			...
			*/
			...
			sess = this.keyManager.getSessKeys(difOur, difTheir);
	...
	KeyManager.prototype = {
		...
		// receiving MAC keys that were generated by that key
		sessHistory: [],
		toReveal: [],

		// Return specified session keys
		// Also record which session keys were used
		getSessKeys: function (i, j) {
			this.sessHistory.push(this.sessKeys[i][j]);
			return this.sessKeys[i][j];
		},
		...
		processReceivedKey: function (intRecipKeyid, intSenderKeyid, mpiSenderNewY) {
			...
			// save forgotten mac keys
			this.saveOldMacKeys();
		},

		// Save old MAC keys to reveal, if needed
		saveOldMacKeys: function () {
			if (!this.sessHistory.length) return;
			var check = this.sessHistory[0];
			if (this.sessKeys[0][0] !== check 
				&& this.sessKeys[0][1] !== check
				&& this.sessKeys[1][0] !== check
				&& this.sessKeys[1][1] !== check
			) {
				this.sessHistory.shift();
				this.toReveal = this.toReveal.concat(Util.wordArrayToByteArray(check.waRecvMacKey));
			}
		},
