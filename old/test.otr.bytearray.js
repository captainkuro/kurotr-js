"use strict";
/**
 * Unit Testing Otr.ByteArray
 * with QUnit
 *
 * @author Khandar William
 *
 * 2012-07-03 write & read tests for Byte, Bool, U/Int32/16, Bytes
 * 2012-07-04 tests for read/writeData & base64
 * 2012-07-05 tests for r/wMPI
 * 2012-07-06 tests for from/toWordArray
 * 2012-07-09 test for byteAt, MPI is ByteArray now
 */

function getRandomInt(min, max) {
	return Math.floor(Math.random() * (max - min + 1)) + min;
}

module('ByteArray');

test('Byte', function () {
	var a, b, i,
		testdata = [];

	for (i=0; i<100; i++) {
		testdata.push(getRandomInt(0, 255));
	}
	a = new Otr.ByteArray();
	for (i=0; i<testdata.length; i++) {
		a.writeByte(testdata[i]);
	}
	equal(a.getLength(), 100, 'Length is 100');
	b = new Otr.ByteArray(a.data);
	for (i=0; i<testdata.length; i++) {
		equal(b.readByte(), testdata[i], 'Success storing byte '+testdata[i]);
	}
	b.reset();
	for (i=0; i<testdata.length; i++) {
		equal(b.byteAt(i), b.readByte(), 'ByteAt has same value as readByte '+b.byteAt(i));
	}
});

test('Bool', function () {
	var testdata = [true, true, false, true, false];
	var a = new Otr.ByteArray();
	for (var i=0; i<testdata.length; i++) {
		a.writeBool(testdata[i]);
	}
	equal(a.getLength(), 5, 'Length is 5');
	var b = new Otr.ByteArray(a.data);
	for (var i=0; i<testdata.length; i++) {
		equal(b.readBool(), testdata[i], 'Success storing boolean '+testdata[i]);
	}
});

test('UInt32', function () {
	var testdata = [];
	var i;
	for (i=0; i<100; i++) {
		testdata.push(getRandomInt(0, 4294967296));
	}
	var a = new Otr.ByteArray();
	for (i=0; i<testdata.length; i++) {
		a.writeUInt32(testdata[i]);
	}
	equal(a.getLength(), 400, 'Length is 400');
	var b = new Otr.ByteArray(a.data);
	for (i=0; i<testdata.length; i++) {
		equal(b.readUInt32(), testdata[i], 'Success storing UInt32 '+testdata[i]);
	}
});

test('Int32', function () {
	var testdata = [];
	var i;
	for (i=0; i<100; i++) {
		testdata.push(getRandomInt(-2147483648, 2147483647));
	}
	var a = new Otr.ByteArray();
	for (i=0; i<testdata.length; i++) {
		a.writeInt32(testdata[i]);
	}
	equal(a.getLength(), 400, 'Length is 400');
	var b = new Otr.ByteArray(a.data);
	for (i=0; i<testdata.length; i++) {
		equal(b.readInt32(), testdata[i], 'Success storing Int32 '+testdata[i]);
	}
});

test('UInt16', function () {
	var testdata = [];
	var i;
	for (i=0; i<100; i++) {
		testdata.push(getRandomInt(0, 65535));
	}
	var a = new Otr.ByteArray();
	for (i=0; i<testdata.length; i++) {
		a.writeUInt16(testdata[i]);
	}
	equal(a.getLength(), 200, 'Length is 200');
	var b = new Otr.ByteArray(a.data);
	for (i=0; i<testdata.length; i++) {
		equal(b.readUInt16(), testdata[i], 'Success storing UInt16 '+testdata[i]);
	}
});

test('Int16', function () {
	var testdata = [];
	var i;
	for (i=0; i<100; i++) {
		testdata.push(getRandomInt(-32768, 32767));
	}
	var a = new Otr.ByteArray();
	for (i=0; i<testdata.length; i++) {
		a.writeInt16(testdata[i]);
	}
	equal(a.getLength(), 200, 'Length is 200');
	var b = new Otr.ByteArray(a.data);
	for (i=0; i<testdata.length; i++) {
		equal(b.readInt16(), testdata[i], 'Success storing Int16 '+testdata[i]);
	}
});

test('Bytes', function () {
	var a, b, c, i,
		testdata = [];

	for (i=0; i<10; i++) {
		testdata.push(getRandomInt(0, 255));
	}

	a = new Otr.ByteArray();
	a.writeBytes(testdata);
	equal(a.getLength(), testdata.length, 'Array length is the same');
	deepEqual(a.toArray(), testdata, 'writeBytes success');

	a.reset();
	c = a.readBytes(testdata.length);
	ok(c instanceof Otr.ByteArray, 'readBytes produce Otr.ByteArray');
	deepEqual(c.toArray(), testdata, 'readBytes success');
	
	b = new Otr.ByteArray();
	a.reset();
	b.writeBytes(a);
	ok(a.equal(b), 'read/writeBytes success');
});

test('Data & MPI', function () {
	var a, b, i,
		testdata = [0, 0, 0, 20],
		testdata2 = [0, 0, 0, 20];

	for (i=0; i<20; i++) {
		testdata.push(getRandomInt(0, 255));
		testdata2.push(getRandomInt(0, 255));
	}

	a = new Otr.ByteArray();
	a.writeData(testdata);
	a.writeData(testdata2);
	a.reset();
	b = a.readData();
	equal(b.getLength(), testdata.length, 'test 1 length is correct');
	deepEqual(b.toArray(), testdata, 'read/writeData test 1');

	b = a.readData();
	equal(b.getLength(), testdata2.length, 'test 2 length is correct');
	deepEqual(b.toArray(), testdata2, 'read/writeData test 2');

	ok(!a.equal(b), 'a and b are not equal');

	a = new Otr.ByteArray();
	a.writeMPI(testdata);
	a.writeMPI(testdata2);
	a.reset();
	b = a.readMPI();
	equal(b.getLength(), testdata.length, 'test 1 length is correct');
	deepEqual(b.toArray(), testdata, 'read/writeMPI test 1');

	b = a.readMPI();
	equal(b.getLength(), testdata2.length, 'test 2 length is correct');
	deepEqual(b.toArray(), testdata2, 'read/writeMPI test 2');
});

test('Base64', function () {
	var testplain = [
			[0x6a, 0xc7, 0x5f, 0x6a, 0xc7, 0x5f, 0x6a, 0xc7, 0x5f],
			[],
			[],
			[],
			[],
			[]
		],
		testbase64 = [
			'asdfasdfasdf',
			'',
			'',
			'',
			'',
			''
		],
		a, b, i;

	for (i=0; i<testplain.length; i++) {
		a = Otr.ByteArray.fromBase64(testbase64[i]);
		deepEqual(a.toArray(), testplain[i], 'fromBase64 test '+i);

		b = new Otr.ByteArray();
		b.writeBytes(testplain[i]);
		equal(b.toBase64(), testbase64[i], 'toBase64 test '+i);
	}
});

test('BigInteger & MPI', function () {
	var b1 = new Otr.BigInteger('12341234123412341234'),
		b2 = new Otr.BigInteger('67896789678967896789'),
		a = new Otr.ByteArray(),
		mpi;

	a.writeMPI(Otr.ByteArray.fromBigInteger(b1));
	a.writeMPI(Otr.ByteArray.fromBigInteger(b2));
	a.reset();
	
	equal(a.readMPI().toBigInteger().toString(), b1.toString(), 'read/writeMPI produce equal BigInteger value');
	equal(a.readMPI().toBigInteger().toString(), b2.toString(), 'read/writeMPI produce equal BigInteger value');
});

test('WordArray', function () {
	var w1, b1, w2,
		i, j, arr;

	i = 100;
	while (i--) {
		arr = [];
		j = 100;
		while (j--) {
			arr.push(getRandomInt(0, 4294967296));
		}
		w1 = CryptoJS.lib.WordArray.create(arr, getRandomInt(1, 100));
		b1 = Otr.ByteArray.fromWordArray(w1);
		w2 = b1.toWordArray();
		equal(b1.toBase64(), w1.toString(CryptoJS.enc.Base64), 'equal base64: '+w1.toString(CryptoJS.enc.Base64));
		equal(w2.toString(), w1.toString(), 'equal wordarray: '+w1.toString());
		equal(w2.sigBytes, w1.sigBytes, 'equal sigBytes');
	}
});